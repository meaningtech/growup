...
test('completes evidence, design, irrigation and costs, then protects persistence behind sign-in', async ({ page }) => {
  ...
  const irrigationControls = page.getByTestId('irrigation-layer-controls');
  const supplyPipes = irrigationControls.getByRole('button', { name: /Supply pipes/ });
  const dripLaterals = irrigationControls.getByRole('button', { name: /Drip laterals/ });
